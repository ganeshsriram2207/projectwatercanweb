...
public class CanAvailability {
	...
	public static void admin() throws SQLException, DBException {
		int stock = 0;
		HomePage hp = null;
		...
		try {
			...
			if (name.equals("admin") && password.equals("admin@2255")) {
				...
				if (stock == 1) {
					...
					can = scan.nextInt();
					if (can > 1000) {
						System.out.println("Please Recheck The Number Available Cans in Our Waterplant \n");
	...
	public static void order() throws SQLException, DBException {
		HomePage hp = null;
		try {
	...
	static void reserve() throws SQLException {
	...
	static void reserveOrder() throws SQLException, InputMismatchException, DBException {
		System.out.println("Do you have to reserve order of?:" + can);
		System.out.println("select 1-->yes/2-->no/3-->modify the order");
		...
		case 3:
			...
			System.out.println("are u sure to order these no of cans" + unit + "" + "if Yes--->1/No--->2");
